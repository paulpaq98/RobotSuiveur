...
void loop() {
  ...
  if (enFonctionnement)
  {
    if (mode == LIBRE)
    {
      ...
      {
        float angleVecteurPoleNord = coordonneeRobot.geoBearing(coordonneeAndroidtest2);
        float distance = coordonneeRobot.longueurArcCercleTerrestre(coordonneeAndroidtest2);
